	...
	<div class="container px-5 py-24 mx-auto">
		<div class="flex flex-col text-center w-full mb-10 md:mb-20">
			...
			<p class="lg:w-2/3 mx-auto leading-relaxed text-base">
				stream CCTV bersumber dari website pemerintah kota maupun provinsi indonesia yang
		...
		<div class="w-full flex justify-center mb-10 md:mb-20">
			{#if data && data.paginate}
			<nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
				<button on:click={prev} class="relative inline-flex items-center rounded-l-md px-2 py-2 text-gray-400 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 focus:z-20 focus:outline-offset-0">
				...
				<button  on:click={next} class="relative inline-flex items-center rounded-r-md px-2 py-2 text-gray-400 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 focus:z-20 focus:outline-offset-0">
		...
		<div class="flex flex-wrap -m-4">
			{#if data}
				{#each data.data as item}
					<Item item={item} source={data.source} />
				{/each}
			{:else}
